...
public class ScapegoatTree<Key extends Comparable<Key>, Value> {
    ...
    private Node put(Node node, Key key, Value val) {
        ...
        if (isCorrectHeightInvariant(node.left, node.right, node)) return node;
    ...
    private boolean isCorrectHeightInvariant(Node leftNode, Node rightNode, Node node) {
        if (height(leftNode) <= alpha * log2(size(leftNode))
                && height(rightNode) <= alpha * log2(size(rightNode))
                && height(node) <= alpha * log2(size(node))) {
            return true;
        }
        return false;
    }
    ...
    private Node balanceNodes(ArrayList<Node> nodes, int lo, int hi) {
        ...
        Node leftNode = balanceNodes(nodes, lo, mid-1);
        Node rightNode = balanceNodes(nodes, mid+1, hi);

        Key midKey = nodes.get(mid).key;
        Value midVal =  nodes.get(mid).val;

        Node node = new Node(midKey, midVal);
        ...
        //node = put(node, midKey, midVal);

        /*if(nodes.get(lo).key.compareTo(nodes.get(mid).key) < 0) {
            balanceNodes(nodes, lo, mid-1);
        } else if (nodes.get(mid).key.compareTo(nodes.get(hi).key) < 0) {
            balanceNodes(nodes, mid+1, hi);
        }
